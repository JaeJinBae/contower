...
<style>
	...
	.contentWrap > .tblWrap > table tr > th:nth-child(7){
		width: 75px;
	}
	.page{
		margin: 15px auto;
...
function get_emptyRoomAll(info){
	var dt;
	$.ajax({
		url:"${pageContext.request.contextPath}/getEmptyroomAll",
		type: "post",
...
function draw_emptyroom(info){
	var json = get_emptyRoomAll(info); 
	var rtype = "";
	var str = "";
	str = "<tr><th>건물명</th><th>호실</th><th>금액</th><th>타입</th><th>난방</th><th>옵션</th><th>PW</th><th>현관/준공</th><th>관리자</th></tr>";
	if(json.list.length == 0){
		str += "<tr><td colspan='9'>등록된 정보가 없습니다.</td></tr>";
		$(".contentWrap > .tblWrap > table").html(str);
	}else{
		$(json.list).each(function(){
			if(this.room_type == "one"){
				rtype = "원룸";
			}else if(this.room_type == "mitwo"){
				rtype = "미니투룸";
			}
			else if(this.room_type == "two"){
				rtype = "투룸";
			}
			else if(this.room_type == "three"){
				rtype = "쓰리룸";
			}
			else if(this.room_type == "store"){
				rtype = "상가";
			}
			else if(this.room_type == "owner"){
				rtype = "주인세대";
			}
			str += "<tr><td><p>"+this.bname+"</p><p>"+this.baddr_old+"</p></td><td>"+this.rno+"호</td><td><p>"+this.hope_price+"</p><p>"+this.selling_type+"</p></td>";
			str += "<td>"+rtype+"</td><td>"+this.heating+"</td><td>"+this.boption+"</td><td>"+this.rpw+"</td><td><p>"+this.bpw+"</p><p>"+this.completion_date+"</p></td><td>"+this.mphone+"</td></tr>";
		});
		$(".contentWrap > .tblWrap > table").html(str);
		...
		for(var i=json.pageMaker.startPage; i<=json.pageMaker.endPage; i++){
			...
			if(json.pageMaker.cri.page == i){
				str += "<li class='active1'><a class='active2' href='page="+i+"&perPageNum=10&searchType="+json.pageMaker.cri.searchType+"&keyword1="+json.pageMaker.cri.keyword1+"&keyword2="+json.pageMaker.cri.keyword2+"'>"+i+"</a></li>";
			}else{
				str += "<li><a href='page="+i+"&perPageNum=10&searchType="+json.pageMaker.cri.searchType+"&keyword1="+json.pageMaker.cri.keyword1+"&keyword2="+json.pageMaker.cri.keyword2+"'>"+i+"</a></li>"
			}
		}
		if(json.pageMaker.next){
			str += "<li><a href='page="+(json.pageMaker.endPage+1)+"&perPageNum=10&searchType="+json.pageMaker.cri.searchType+"&keyword1="+json.pageMaker.cri.keyword1+"&keyword2="+json.pageMaker.cri.keyword2+"'>&raquo;</a></li>";
		}
		str += "</ul>";
	...
	<div class="allWrap">
		...
		<div class="sectionWrap">
			<div class="sectionTitle">
				<p><img src="${pageContext.request.contextPath}/resources/images/icon_plus_cr.png"><span>공실정보</span><img src="${pageContext.request.contextPath}/resources/images/icon_line.png"></p>
			</div>
			<div class="section">
